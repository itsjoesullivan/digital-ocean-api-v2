...
var $ = cheerio.load(fs.readFileSync('./documentation-2016-01-11.html', 'binary'));
...
$("nav.sidebar > ul > li").each(function(i, el) {
  ...
  $(el).find('> ul > li > a').each(function(i, vel) {
    ...
    var itemObj = {
      name: camel(name.replace(/(a |an |the )/ig, '')),
      path: path,
      method: method
    };
    fs.appendFileSync(mdName, "- [" + itemObj.name + "](" + linkUrl + ")\n", 'binary');
    fs.appendFileSync(mdName, "\n`api." + obj.name + "." + itemObj.name + "`\n", 'binary');
    ...
    $thing.find('tbody').first().find('tr').each(function(i, el) {
      ...
      if (row.find('td').length === 4) {
        ...
        propObj = {
          ...
          required: $(row.find('td')[3]).text() === true
        };
